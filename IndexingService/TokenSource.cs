...
namespace IndexingService
{
    public sealed class TokenSource
    {
        ...
        public char[] Buffer { set; get; } = new char[256];
            ...
            while ((r = _reader.Read()) != -1) // EOF
            {
                ...
                if (ch == '\r' || ch == '\n' || char.IsWhiteSpace(ch) || (char.IsPunctuation(ch) && ch != '\''))
                {
                    if (Size > 0)
                    {
                        ...
                        {
                            if (int.TryParse(this.ToString(), out var dateAsInt))
                            {
                                if (Core.IsLastCharPunctuation && Core.lstDate.Count == 1)
                                {
                                    Replace(dateAsInt);
                                    Core.IsLastCharPunctuation = false;
                                    Core.lstDate.Clear();
                                }
                                else
                                {
                                    Core.lstDate.Add(dateAsInt);

                                    Core.IsLastCharPunctuation = true;
                                }
                            }
                        }
                        else
        ...
        private void Replace(int dateAsInt)
        {
            Buffer = new char[256];
            Size = 0;

            for (int i = 0; i < dicMonths[dateAsInt].Length; i++)
            {
                Buffer[Size++] = dicMonths[dateAsInt][i];
            }
        }
        ...
        {
            var results = new HashSet<TermSegment>();
            while (Next())
            {
                if (filter(this))
                {
                    var term = new TermSegment(
                        Buffer,
                        Size
                        );
